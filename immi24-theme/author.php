...
<main id="primary" class="site-main max-w-[1298px] mx-auto px-3 md:px-4 lg:px-24 py-6 md:py-12 mb-12">

    <!-- Header Section (Author Bio) -->
    <div class="mb-8 md:mb-12 pb-6 md:pb-8 border-b border-slate-200">
        <div class="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
            <div class="w-24 h-24 md:w-32 md:h-32 shrink-0 rounded-full overflow-hidden bg-slate-100 shadow-sm border border-slate-200">
                <?php echo get_avatar( $author_id, 256, '', '', array( 'class' => 'w-full h-full object-cover' ) ); ?>
            </div>
            <div class="text-center md:text-left flex-1 mt-2 md:mt-0">
                <h1 class="text-3xl md:text-4xl lg:text-5xl font-extrabold headline-font text-slate-900 mb-3 md:mb-4">
                ...
                if ( $author_bio ) : ?>
                    <p class="text-slate-600 text-sm md:text-[17px] leading-relaxed max-w-3xl mb-5">
                        <?php echo wp_kses_post( $author_bio ); ?>
                    </p>
                <?php endif; ?>

                <div class="inline-flex items-center gap-2 text-[10px] md:text-xs font-bold uppercase tracking-widest text-primary bg-primary/10 px-4 py-2 rounded-full">
                    <span class="material-symbols-outlined text-[16px]">article</span>
                    <?php 
                    $post_count = count_user_posts( $author_id );
                    echo sprintf( _n( '%s Article Published', '%s Articles Published', $post_count, 'immi24' ), number_format_i18n( $post_count ) ); 
                    ?>
                </div>
            </div>
        </div>
                        ...
                        <article id="post-<?php the_ID(); ?>" <?php post_class( 'group flex flex-col space-y-3 md:space-y-4 border border-transparent hover:border-slate-100 pb-4 rounded-xl transition-all hover:shadow-lg bg-white relative' ); ?>>
                            ...
                            <a href="<?php the_permalink(); ?>" class="block">
                                <div class="relative aspect-[16/10] overflow-hidden rounded-t-xl w-full bg-slate-100">
                                    ...
                                    <?php else : ?>
                                        <img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 400 300'%3E%3Crect width='400' height='300' fill='%23f1f5f9'/%3E%3Cpath d='M150 120a30 30 0 1 1 0 60 30 30 0 0 1 0-60zm130 90H120l50-50 20 20 60-60 30 30v60z' fill='%23cbd5e1'/%3E%3C/svg%3E" alt="" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                    <?php endif; ?>
                                    
                                    <?php
                                    $categories = get_the_category();
                                    if ( ! empty( $categories ) ) {
                                        echo '<div class="absolute top-3 left-3 bg-primary px-3 py-1 text-[10px] font-bold text-white uppercase tracking-widest rounded-sm shadow-md">' . esc_html( $categories[0]->name ) . '</div>';
                                    }
                                    ?>
                                </div>
                            </a>
	...
	<?php else : ?>
        <section class="no-results not-found text-center py-20">
            <h1 class="text-3xl font-bold text-slate-900 mb-4"><?php esc_html_e( 'No Articles Found', 'immi24' ); ?></h1>
            <p class="text-slate-600 mb-8"><?php esc_html_e( 'This author has not published any articles yet.', 'immi24' ); ?></p>
