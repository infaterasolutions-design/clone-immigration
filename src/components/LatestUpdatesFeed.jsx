  ...
  return (
    <div className="lg:col-span-2 space-y-3 md:space-y-4">
      ...
      {visibleArticles.map((article, index) => (
        ...
        >
          <Link href={article.cluster_slug ? `/${article.cluster_slug}/${article.slug}` : `/${article.slug}`} className="flex-grow min-w-0">
          ...
          <Link href={article.cluster_slug ? `/${article.cluster_slug}/${article.slug}` : `/${article.slug}`} className="w-[110px] h-[75px] md:w-[190px] md:h-[125px] overflow-hidden flex-shrink-0 block bg-slate-100 relative">
