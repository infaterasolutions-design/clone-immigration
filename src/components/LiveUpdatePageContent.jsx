...
const LiveUpdateCard = ({ update, pageUrl }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [showShareMenu, setShowShareMenu] = useState(false);
  const [copied, setCopied] = useState(false);

  const handleShare = (platform) => {
    const url = `${window.location.origin}${pageUrl}#${update.id}`;
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 md:p-6 shadow-sm relative">
          ...
          <div className="absolute -bottom-5 right-4 md:right-8 z-20 flex flex-col items-end">
            {showShareMenu && (
              ...
              >
                <button onMouseDown={() => handleShare('facebook')} title="Share on Facebook" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-[#1877F2] transition-colors">
                  <span className="font-bold text-lg leading-none mt-[-2px]">f</span>
                </button>
                <div className="w-px h-5 bg-slate-200 mx-0.5" />
                <button onMouseDown={() => handleShare('twitter')} title="Share on X" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-900 transition-colors">
                  <span className="font-bold text-lg leading-none mt-[-2px]">𝕏</span>
                </button>
                <div className="w-px h-5 bg-slate-200 mx-0.5" />
                <div className="relative">
                  <button onMouseDown={() => handleShare('copy')} title="Copy Link" className={`w-8 h-8 flex items-center justify-center rounded-full transition-colors ${copied ? 'bg-green-50 text-green-600' : 'hover:bg-slate-100 text-slate-600'}`}>
              ...
              onClick={() => {
                ...
                if (isMobile && navigator.share) {
                  navigator.share({
                    ...
                    url: `${window.location.origin}${pageUrl}#${update.id}`,
...
/**
 * LiveUpdatePageContent — Shared component for rendering any live event.
 * 
 * Props:
 *  - eventId: string — The live event ID to fetch
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/" className="bg-primary text-white px-6 py-3 font-bold text-sm uppercase tracking-wider hover:opacity-90 transition-all rounded-md">
            ← Back to Home
          </Link>
        </div>
      </div>
        ...
        <section className="col-span-1 lg:col-span-7">
          <header className="mb-8 md:mb-12">
            ...
            <div className="flex items-center gap-4 md:gap-6 mb-6 md:mb-10 text-on-surface-variant flex-wrap">
              {event.authors && event.authors.length > 0 ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <div style={{ display: 'flex', flexShrink: 0, minWidth: 'fit-content' }}>
                      {event.authors.map((author, index) => (
                        <div key={index} className="author-avatar" style={{ width: '40px', height: '40px', minWidth: '40px', minHeight: '40px', overflow: 'hidden', border: '2px solid white', backgroundColor: '#f1f5f9', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)', flexShrink: 0, position: 'relative', marginLeft: index > 0 ? '-12px' : '0px', zIndex: event.authors.length - index }}>
                          <img src={author.image || "/images/u1.jpg"} alt={author.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        </div>
                      ))}
                    </div>
                    <div className="ml-3 text-[15px] flex-1 min-w-0">
                      <span className="text-slate-500">By </span>
                      <span className="font-bold text-slate-900 truncate block">{event.authors.map((a) => a.name).join(" and ")}</span>
                    </div>
                  </div>
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div className="author-avatar" style={{ width: '40px', height: '40px', minWidth: '40px', minHeight: '40px', overflow: 'hidden', flexShrink: 0 }}>
                      <img src={event.author.image || "/images/u1.jpg"} alt={event.author.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    <div>
              ...
              <div className="text-xs md:text-sm font-medium">{event.date}</div>
            </div>
            ...
            {event.headerContext && (
              <div className="relative">
                ...
                {event.headerContext.length > 300 && (
                  <div className="mt-4">
                     <button onClick={() => setIsHeaderExpanded(!isHeaderExpanded)} className="text-primary font-bold text-[12px] uppercase tracking-widest hover:text-blue-700 transition-colors flex items-center gap-1">
            ...
            {visibleUpdates.length > 0 ? (
              <>
                ...
                {visibleUpdates.map((update) => (
                  <LiveUpdateCard key={update.id} update={update} pageUrl={pageUrl} />
                ))}
              </>
            ) : (
          ...
          {hasMore && (
            <div className="mt-12 md:mt-16 flex justify-center">
              <button onClick={handleLoadMore} disabled={isLoading} className={`bg-primary text-on-primary px-6 md:px-8 py-3 md:py-4 font-headline font-bold text-xs md:text-sm tracking-widest uppercase hover:bg-blue-800 transition-all flex items-center gap-3 rounded-full w-full sm:w-auto justify-center shadow-md ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}>
